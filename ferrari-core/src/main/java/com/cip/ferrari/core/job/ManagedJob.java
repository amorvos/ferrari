...
public class ManagedJob implements Runnable {
    ...
    public ManagedJob(String uuid, String jobName, List<String> returnUrllist, Date beginTime,
    ...
    public void init() {
        ...
        if (null == returnUrllist || returnUrllist.isEmpty()) {
            throw new IllegalArgumentException("member returnUrllist is null or empty!");
        }
        if (jobParam.isEmpty()) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Fail to find the run class(" + className + ") of job!");
        }
        // 检查运行的方法是否存在
        getMethod(runClass, methodName);
        if (null == runMethod) {
            throw new IllegalArgumentException("the run method(" + methodName + ") of job is not existed!");
        }
