...
public class FeedbackHandler {
    ...
    private final BlockingQueue<ManagedJob> feedbackQueue = new LinkedBlockingQueue<ManagedJob>(10000);

    private final BlockingQueue<RetrySendJob> resendQueue = new LinkedBlockingQueue<RetrySendJob>(20000);

    private final long[] resendTimePoints = { 1000 * 5, 1000 * 30, 1000 * 120, 1000 * 240 };
    ...
    private void dealSendBack() {
        while (true) {
            ...
            try {
                ...
                // http发送
                boolean sendSuccess = doSend(job, httpParams, job.getReturnUrllist());

                if (!sendSuccess) {// 反馈失败
                    throw new RuntimeException("send feedback failed," + job.toString());
                }
            } catch (Throwable t) {
            ...
            try {
                ...
                if (System.currentTimeMillis() >= retryJob.getResendTime()) {
                    if (retryJob.getRetryTimes() >= resendTimePoints.length) {
                        logger.error("Fail to resend Job in many times," + retryJob);
                        continue;
                    }
                ...
                logger.error("job feedback retrySend happens exception." + retryJob.toString(), t);
    ...
    private boolean doSend(ManagedJob job, Map<String, String> httpParams, List<String> urlList) {
        for (String url : urlList) {
            try {
                ...
                if (RESPONSE_OK.equalsIgnoreCase(resp)) {
                    if (logger.isInfoEnabled()) {
                        logger.info("send feedback{} success with job{}.", urlTmp, job.toString());
                    }
                    return true;
                }
                ...
                logger.error("send feedback happens exception,url=" + url + ",job:" + job.toString(), e);
            }
        }
        return false;
    }
    ...
    private String getFeedbackJson(ManagedJob job) {
        ...
        if (job.getRunThrowable() != null) {
            ...
            feedback.setErrormsg(StringUtils.substring(builder.toString(), 0, 2048));
        }
    ...
    private class RetrySendJob {
        ...
        public RetrySendJob(ManagedJob job) {
            this.retryTimes = 0;
            this.resendTime = System.currentTimeMillis() + resendTimePoints[retryTimes];
            this.job = job;
        }
        ...
        public void addRetryTimesAndSetSendTime() {
            ...
            if (retryTimes >= resendTimePoints.length) {
                this.resendTime = System.currentTimeMillis() + resendTimePoints[resendTimePoints.length - 1];
            } else {
                this.resendTime = System.currentTimeMillis() + resendTimePoints[retryTimes];
            }
        }
